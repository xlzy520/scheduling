export default {
  ...
  float: {pattern: /^([0]*?(\.)?([1-9]\d?|0[1-9]))(\.\d{1,2})?$/, message: '请输入最多两位小数，且不为0', trigger: 'change'}, //正整数或小数
  number: {pattern: /^\d*[1-9]+\d*$/, message: '请输入正整数', trigger: 'change'}, //正整数,不为0
  float2: {pattern: /^([0]*?(\.)?([1-9]\d?|0[1-9]))(\.\d{1,2})?$/, message: '请输入两位小数', trigger: 'change'}, //只能输入两位小数
