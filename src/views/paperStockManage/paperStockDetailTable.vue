    ...
    data() {
      return {
        ...
        tableColumns: [
          ...
          {label: '原纸供应商', prop: 'supplierName', width: 120, sortable: true,
            sortMethod: (a, b)=>this.sort(a, b, 'supplierName')},
          {label: '纸筒编号', prop: 'paperTubeNumber', width: 160, sortable: true},
          {label: '原纸代码', prop: 'paperCode', width: 120, sortable: true},
          // {label: '原纸类型', prop: 'paperType', width: 120,sortable: true,,
          //   formatter: (row, index, cur) => {
          //     let obj = this.$enum.paperType._swap[cur] || {};
          //     return obj.label || '';
          //   }
          // },
          {label: '原纸名称', prop: 'paperName', width: 120, sortable: true,
          ...
          {label: '克重（g）', prop: 'gram', width: 120, sortable: true},
          {label: '门幅（mm）', prop: 'paperSize', width: 120, sortable: true},
          {label: '重量（Kg）', prop: 'weight', width: 120, sortable: true},
          {label: '长度（m）', prop: 'length', width: 120, sortable: true},
          {label: '仓库', prop: 'warehouseName', width: 120, sortable: true,
            sortMethod: (a, b)=>this.sort(a, b, 'warehouseName')},
          {label: '库区', prop: 'warehouseAreaName', width: 120, sortable: true,
            sortMethod: (a, b)=>this.sort(a, b, 'warehouseAreaName')},
          {label: '入库类型', prop: 'storageType', width: 120, sortable: true,
